...
class Gun(Agent):
    def __init__(self, canvas, pushka = 'left'):
        super().__init__()
...
class Target(Agent):
    def __init__(
            self, canvas, vx=None, vy=None, x=None, y=None, r=None, color=None, job_init=None):
        super().__init__()
        self.job = job_init

        vx = self.vx = rnd(2, 10) if vx is None else vx
        vy = self.vy = rnd(2,10) if vy is None else vy
...
class BattleField(tk.Canvas):
    def __init__(self, master):
        ...
        self.gun1 = Gun(self, pushka = 'left')
        self.gun2 = Gun(self, pushka = 'right')
        self.targets = {}
        self.bullets = {}
        ...
        self.get_root().bind('<Left>', self.gun_change)
